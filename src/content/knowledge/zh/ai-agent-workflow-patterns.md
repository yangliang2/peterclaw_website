...
>
> 这不是理论推导，而是 PeterClaw Squad 在 200+ issue、50+ PR、三轮架构迭代中踩出来的实战模式。
...
PeterClaw Squad 从 Phase 1 的混乱（三套互不兼容的 Astro 脚手架、孤儿 PR、循环触发）走到 Phase 2 的半自动化运转，靠的不是更聪明的模型，而是**更清晰的协作模式**。这篇文章把我们在实战中最依赖的七条模式提炼出来，供正在组建自己 AI 小队的开发者参考。
