...
批量规划上线之后，团队有了自动触发机制。但一个新的问题很快浮现：**任务发出了，谁来检查它们有没有被正确执行？**
...
| 巡检类型 | 频率 | 职责 | 对应 Issue |
|----------|------|------|-----------|
| **PR 巡检** | 每 4 小时 | 扫描 `in_review` issue，检查 PR 状态，合并合格的，催促滞后的 | autopilot #2 |
| **Backlog 扫描** | 每 4 小时 | 统计 `in_progress + todo` 总数，若 < 3 则触发新任务 | autopilot #3 |
| **Brainstorming** | 每 8 小时 | 补充 backlog 弹药，确保未启动项 ≥ 5 | autopilot #1 |
...
Bug 第一次暴露是在 autopilot #1 的第三次运行。

Claude 2号 发现连续两次 backlog 扫描都返回 `no_action`——「活跃任务已达上限」。但手动检查后发现，真正在做的功能任务只有 1 个（cursor 1号 修 About 页面），另外 2 个是 autopilot #1（Brainstorming）和 autopilot #2（PR 巡检）。
...
它没有区分「人类/Agent 的功能任务」和「autopilot 自身的管理任务」。在 Multica 的视角里，这三个 autopilot 任务和其他功能任务一样，都是合法的 `in_progress` issue。
...
Backlog 里躺着 7 个已规划的任务，但自动触发扫描连续 12 小时没有提任何一个。人类负责人手动检查发现一个 RSS Feed 接入任务明明优先级很高，却一直卡在 `[ ]`。不是任务不重要，是系统「觉得」团队很忙。
...
这三个 autopilot 现在会正确地忽略彼此的存在。Backlog 扫描在 2026-05-24 下午 16:00 的巡检中，成功将该 RSS 任务从 `[ ]` 提升为 `[→]`，cursor 1号 在 4 分钟内被触发并开始工作。
